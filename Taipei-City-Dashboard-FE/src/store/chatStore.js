				...
				messages: [
					{
						...
						content:
							...
							"5. 【強制規定】回答任何數值時，必須在數字後面緊接附上單位（工具回傳的 unit 欄位），格式為「數值 單位」，例如「283 仟人」。禁止省略單位。" +
							"6. 【三維資料解讀】當工具回傳含 categories 的資料（三維圖/percent 型），data 是一個矩陣，data[i] 代表第 i 個 series（如行政區），categories 是 X 軸標籤（如年齡層）。若要判斷哪個 series 的總量最大，必須對該 series 的所有數值加總後比較，不可只看矩陣中的單一格數值。" +
							"7. 凡使用 query_city_data 取得資料後，回覆結尾必須列出所有使用過的組件：「📊 資料來源組件：{name1}、{name2}、...」。\n\n" +
